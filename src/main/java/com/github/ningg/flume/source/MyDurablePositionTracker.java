...
public class MyDurablePositionTracker implements PositionTracker {
	 ...
	 * @throws IOException
	 */
	public static MyDurablePositionTracker getInstance(File trackerFile, String target) throws IOException {

		if (!trackerFile.exists()) {
			return new MyDurablePositionTracker(trackerFile, target);
		}

		// exists
		// 在这个方法中会读一批数据 initReader();
		...
		tmpMeta.delete();
		MyDurablePositionTracker tmpTracker = new MyDurablePositionTracker(tmpMeta, existingTarget);
		...
		MyDurablePositionTracker newTracker = new MyDurablePositionTracker(trackerFile, existingTarget);
		return newTracker;
	}
	 ...
	 * @throws IOException
	 */
	public MyDurablePositionTracker(File trackerFile, String target) throws IOException {
		...
		DatumWriter<TransferStateFileMeta> dout = new SpecificDatumWriter<TransferStateFileMeta>(TransferStateFileMeta.SCHEMA$);

		DatumReader<TransferStateFileMeta> din = new SpecificDatumReader<TransferStateFileMeta>(TransferStateFileMeta.SCHEMA$);
		...
		if (trackerFile.exists()) {
			...
			this.target = reader.getMetaString("file"); // file name ：
														// /home/song/workspace/bigdata/firstMavenTest/logs/info.log
		} else {
			// create the file
			this.target = target;
